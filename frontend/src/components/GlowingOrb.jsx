    ...
    <div className={`relative flex items-center justify-center select-none ${orbSize} ${className}`}>
      ...
      <div
        className={`absolute inset-[-40%] rounded-full blur-2xl transition-all duration-700 pointer-events-none ${
          state === 'thinking'
            ? 'bg-gradient-to-r from-red-600/40 via-amber-500/40 to-yellow-500/40 animate-spin scale-110'
            : state === 'generating'
            ? 'bg-gradient-to-r from-red-500/50 via-amber-400/50 to-rose-600/50 scale-125 animate-pulse'
            : state === 'speaking'
            ? 'bg-red-600/35 scale-115 animate-ping duration-1000'
            : 'bg-gradient-to-r from-red-600/25 to-amber-500/20 scale-95 opacity-80'
        }`}
      />
      ...
      <div
        className={`absolute inset-0 rounded-full border border-amber-400/30 transition-all duration-500 pointer-events-none ${
          state === 'thinking'
            ? 'animate-[spin_4s_linear_infinite] scale-125 border-dashed border-red-500/60'
            : state === 'generating'
      ...
      {(state === 'thinking' || state === 'generating') && (
        <div className="absolute inset-[-15%] rounded-full border border-red-500/40 border-dotted animate-[spin_6s_linear_infinite_reverse] pointer-events-none" />
      )}
      ...
      <div
        ...
        style={{
          background:
            state === 'generating'
              ? 'radial-gradient(circle at 35% 30%, #fff 0%, #fbbf24 25%, #e50914 60%, #b80710 100%)'
              : state === 'thinking'
              ? 'radial-gradient(circle at 30% 25%, #ffffff 0%, #fbbf24 30%, #e50914 65%, #121216 100%)'
              : state === 'speaking'
              ? 'radial-gradient(circle at 40% 30%, #ffffff 0%, #fbbf24 35%, #e50914 70%, #990000 100%)'
              : 'radial-gradient(circle at 35% 30%, #ffffff 0%, #fbbf24 20%, #e50914 50%, #08080a 100%)',
          boxShadow:
            state === 'thinking'
              ? '0 0 35px rgba(229, 9, 20, 0.6), inset 0 0 20px rgba(255, 255, 255, 0.8)'
              : '0 0 30px rgba(255, 184, 0, 0.5), inset 0 0 15px rgba(255, 255, 255, 0.7)'
        }}
      >
      ...
      {state === 'thinking' && (
        <>
          <div className="absolute w-2 h-2 rounded-full bg-red-500 shadow-[0_0_8px_#e50914] animate-[spin_2.5s_linear_infinite] -top-1 left-1/2 -translate-x-1/2" />
