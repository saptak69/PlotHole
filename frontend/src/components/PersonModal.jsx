...
export default function PersonModal({ personId, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-black/85 flex items-center justify-center p-4 backdrop-blur-2xl animate-fade-in">
      <div 
        className="w-full max-w-3xl rounded-3xl overflow-hidden border border-white/12 bg-[#080c14] text-slate-100 shadow-[0_25px_70px_rgba(0,0,0,0.9),0_0_30px_rgba(0,245,160,0.15)] max-h-[90vh] flex flex-col"
      ...
      >
        ...
        {loading ? (
          <div className="p-16 text-center space-y-3">
            <Loader2 className="w-8 h-8 animate-spin mx-auto text-[#00f5a0]" />
            <p className="font-mono text-xs text-[#00f5a0] font-bold uppercase tracking-widest">FETCHING ARTIST ARCHIVE...</p>
          </div>
        ) : person ? (
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              ...
              <div className="space-y-2.5 flex-1">
                ...
                {person.known_for_department && (
                  <span className="inline-block px-2.5 py-0.5 bg-[#00f5a0]/15 text-[#00f5a0] border border-[#00f5a0]/30 text-[10px] font-mono font-bold uppercase rounded-full">
                    {person.known_for_department}
                  </span>
                )}
                {person.birthday && (
                ...
                {person.biography && (
                  <p className="text-xs font-sans text-slate-300 line-clamp-4 leading-relaxed bg-black/40 p-3.5 rounded-xl border border-white/5">
                    {person.biography}
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                {topCredits.map((credit) => (
                  <div
                    ...
                    className="group bg-white/5 hover:bg-[#0e1422] border border-white/8 hover:border-[#00f5a0]/40 rounded-2xl p-2 text-center cursor-pointer transition-all hover:-translate-y-1"
                  >
                    <img
                    ...
                    />
                    <h4 className="font-display font-bold text-xs truncate text-slate-200 group-hover:text-[#00f5a0]">
